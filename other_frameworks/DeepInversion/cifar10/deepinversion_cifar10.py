...
def get_images(net, bs=256, epochs=1000, idx=-1, di_var_scale=0.00005,
               net_student=None, prefix=None, competitive_scale=0.01, train_writer = None, global_iteration=None,
               optimizer = None, inputs = None, r_feature_weight = 0.0, random_labels = False, di_l2_scale=0.0):
    '''
    ...
    args in:
        ...
        di_var_scale: the scaling factor for variance loss regularization. this may vary depending on bs
        ...
        r_feature_weight: weight for r_feature_regularization
        random_labels: sample labels from random distribution or use columns of the same class
        di_l2_scale: coefficient for L2 loss on input
    return:
    ...
    for epoch in range(epochs):
        ...
        inputs_jit = torch.roll(inputs, shifts=(off1,off2), dims=(2,3)).float()
        ...
        loss = loss + r_feature_weight*loss_distr # best for noise before BN

        # l2 loss
        if 1:
            loss = loss + di_l2_scale * torch.norm(inputs_jit, 2)
    ...
    if train_writer is not None:
        ...
        train_writer.add_scalar('gener_loss_var', (di_var_scale*loss_var).item(), global_iteration)
...
if __name__ == "__main__":
    ...
    net_teacher = torchvision.models.resnet34(weights=torchvision.models.ResNet34_Weights)
    net_teacher.fc = nn.Linear(net_teacher.fc.in_features, 10)
    ...
    checkpoint = {k[13:]: v for k, v in checkpoint.items()}
    net_teacher.load_state_dict(checkpoint, strict=True)
    ...
    inputs = get_images(net=net_teacher, bs=args.bs, epochs=args.iters_mi, idx=batch_idx,
                        ...
                        optimizer=optimizer_di, inputs=inputs, r_feature_weight=args.r_feature_weight,
                        di_var_scale=args.di_var_scale, random_labels=False, di_l2_scale=args.di_l2_scale)
