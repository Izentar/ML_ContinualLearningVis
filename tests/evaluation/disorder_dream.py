...
class DisorderDream():
    ...
    def _select_rand_image(self, dataset, used_class):
        indices = select_class_indices_tensor(torch.IntTensor(dataset.targets), used_class)
        index = random.choice(indices)
        return dataset[index]

    def __call__(self, model, used_class, dataset, logger, dream_transform, batch_size, objective_f=None, loss_f=None):
        constructed_dreams = []
        ...
        # select image
        image, _ = self._select_rand_image(dataset, used_class)
        ...
        dream_module = CustomDreamDataModule(
            ...
            dream_objective_f=dream_objective_latent_lossf_creator(logger),
            target_processing_f=self.task_processing_decorator(DisorderDream.target_processing_custom_creator(point)),
            dreams_per_target=1,
        ...
        self._log(constructed_dreams=constructed_dreams, real_image=image, logger=logger, label=label)

    def _log(self, constructed_dreams, real_image, logger, label):
